...
#   3. `tauri build --bundles app` → Niash.app (resources are copied in).
...
echo "==> [1/5] PyInstaller: bundling nish-server ($TRIPLE)"
"$PLATFORM/.venv/bin/pyinstaller" --noconfirm --clean \
  --distpath "$HERE/dist" --workpath "$HERE/build" "$HERE/nish-server.spec"
...
rm -rf "$GUI/src-tauri/binaries/sidecar" "$GUI/src-tauri/binaries/nish-server-$TRIPLE"
...
cp -RL "$HERE/dist/nish-server" "$GUI/src-tauri/binaries/sidecar"
...
chmod +x "$GUI/src-tauri/binaries/sidecar/nish-server"
...
if [ -n "${APPLE_SIGNING_IDENTITY:-}" ]; then
  ...
  find "$SIDECAR" -type f ! -name "nish-server" \
  ...
  codesign --force --sign "$APPLE_SIGNING_IDENTITY" --timestamp --options runtime \
    --entitlements "$GUI/src-tauri/entitlements.plist" "$SIDECAR/nish-server"
fi
